...
if 'ckeditor' in settings.INSTALLED_APPS:
    # Confirm CKEDITOR_STATIC_PREFIX setting has been specified.
    try:
        settings.CKEDITOR_STATIC_PREFIX
    except AttributeError:
        raise ImproperlyConfigured("django-ckeditor requires \
                CKEDITOR_STATIC_PREFIX setting. This setting specifies a URL \
                prefix to the ckeditor JS and CSS media (not uploaded media). \
                Make sure to use a trailing slash: CKEDITOR_STATIC_PREFIX = \
                '/static/ckeditor/'")
